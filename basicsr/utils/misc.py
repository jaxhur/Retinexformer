    ...
    if opt['path']['resume_state']:
        ...
        for network in networks:
            ...
            if opt['path'].get('ignore_resume_networks') is None or (
                ...
                # ImageCleanModel stores generator checkpoints as
                # ``<iteration>_G.pth`` so they can be passed directly to the
                # standalone test script. Keep the original convention for
                # any other network type.
                filename = (f'{resume_iter}_G.pth' if basename == 'g' else
                            f'net_{basename}_{resume_iter}.pth')
                opt['path'][name] = osp.join(opt['path']['models'], filename)
